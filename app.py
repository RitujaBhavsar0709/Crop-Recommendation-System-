...
# # ---- FILES (District Level) ----
# MERGED_CROP_FILE = "merged_crop_price_enhanced_full.csv"   # or merged_crop_price.csv
# STATES_FILE = "states_maha.csv"
# DISTRICTS_FILE = "districts_maha.csv"
# DISTRICT_DEFAULTS_FILE = "district_defaults_maha.csv"
# CROP_MASTER_FILE = "crop_master_maha.csv"
...
#     final_df = pd.read_csv("merged_crop_price.csv")
# else:
#     raise FileNotFoundError("Merged crop file missing!")

# # State & District & Defaults
# if not os.path.exists(STATES_FILE) or not os.path.exists(DISTRICTS_FILE) or not os.path.exists(DISTRICT_DEFAULTS_FILE):
#     raise FileNotFoundError("states_maha.csv, districts_maha.csv or district_defaults_maha.csv missing.")

# states_df = pd.read_csv(STATES_FILE)
# districts_df = pd.read_csv(DISTRICTS_FILE)
# district_defaults_df = pd.read_csv(DISTRICT_DEFAULTS_FILE)

# # Optional crop master
# crop_master_df = pd.read_csv(CROP_MASTER_FILE) if os.path.exists(CROP_MASTER_FILE) else None
...
#     # Case 1: state_id exists
#     if "state_id" in states_df.columns and "state_id" in districts_df.columns:
#         state_id = states_df[states_df["state_name"] == selected_state]["state_id"].values[0]
#         districts = list(districts_df[districts_df["state_id"] == state_id]["district_name"].unique())

#     # Case 2: fallback by state name
#     elif "state" in districts_df.columns:
#         districts = list(districts_df[districts_df["state"] == selected_state]["district_name"].unique())
...
#         ].iloc[0].to_dict()
#     except:
#         defaults = {"N": 80, "P": 40, "K": 40, "temperature": 28,
#                     "humidity": 60, "ph": 6.7, "rainfall": 500}
...
#         # Read user inputs
#         N = safe_float(request.form.get("N"), defaults["N"])
#         P = safe_float(request.form.get("P"), defaults["P"])
#         K = safe_float(request.form.get("K"), defaults["K"])
#         temperature = safe_float(request.form.get("temperature"), defaults["temperature"])
#         humidity = safe_float(request.form.get("humidity"), defaults["humidity"])
#         ph = safe_float(request.form.get("ph"), defaults["ph"])
#         rainfall = safe_float(request.form.get("rainfall"), defaults["rainfall"])
#         farm_size = safe_float(request.form.get("farm_size"), 1.0)
...
#             "ph": ph,
#             "rainfall": rainfall,
#             "yield_per_hectare": final_df["yield_per_hecture"].mean()
#         }])

#         probs = model.predict_proba(input_df)[0]
#         classes = model.classes_

#         # No taluka filter → use district only
#         crop_prob_map = {c: probs[i] for i, c in enumerate(classes)}
